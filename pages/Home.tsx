    ...
    <div className="flex flex-col space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
        ...
        <button 
          onClick={handleSpin}
          disabled={isSpinning}
          className={`w-full py-5 rounded-2xl text-xl font-black shadow-lg transition-all active:scale-95 flex items-center justify-center space-x-3 ${isSpinning ? 'bg-gray-200 text-gray-400' : 'bg-orange-500 hover:bg-orange-600 text-white'}`}
        >
          {isSpinning ? (
            <>
      ...
      {!winner && !isSpinning && getMeals().filter(m => m.isActive && (m.mealType === 'Both' || m.mealType === mealType) && (category === 'All' || m.category === category)).length === 0 && (
