...
const AddMeal: React.FC<AddMealProps> = ({ lang, onSuccess, onCancel, editId }) => {
  ...
  const existingMeal = editId ? getMeals().find(m => m.id === editId) : null;

  const [formData, setFormData] = useState<Omit<Meal, 'id'>>({
    name: existingMeal?.name || '',
    category: existingMeal?.category || 'Medium',
    mealType: existingMeal?.mealType || 'Both',
    cookingTime: existingMeal?.cookingTime || 30,
    difficulty: existingMeal?.difficulty || 'Medium',
    priceLevel: existingMeal?.priceLevel || 'Medium',
    proteinType: existingMeal?.proteinType || 'Mixed',
    cuisine: existingMeal?.cuisine || (lang === 'bg' ? 'Българска' : 'Bulgarian'),
    ...
    <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 animate-in slide-in-from-right-10 duration-300">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex space-x-3 pt-6">
          ...
          <button 
            type="submit"
            className="flex-1 py-4 bg-orange-500 hover:bg-orange-600 text-white font-black rounded-2xl transition-all shadow-lg uppercase tracking-widest text-xs active:scale-95"
          >
